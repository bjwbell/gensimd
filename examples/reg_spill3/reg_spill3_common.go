...
//go:generate gensimd -fn "regspill3" -outfn "regspill3" -f "reg_spill3_simd.go" -o "reg_spill3_amd64.s" -goprotofile "reg_spill3_simd_proto.go"

func regspill3Ref(x, y []simd.I32x4) int32 {
	if len(x) != len(y) {
		return -1
	}
...
func main() {
	...
	val := regspill3(x, y)
	if val != regspill3Ref(x, y) {
		fmt.Printf("SIMD %v != reference (%v)\n", val, regspill3Ref(x, y))
		errors++
	}
